...
type
  TDxInstallOption = (dxioAddBrowsingPath, dxioNativeLookAndFeel,
    dxioCompileWin64Library, dxioInstallToCppBuilder, dxioDebug);
  ...
  TDxUpdateProgressEvent = procedure(IDE: TDxIDE;
    Component: TDxComponentProfile; const Task, Target: String) of object;
  ...
  private
    ...
    procedure SetThirdPartyComponents(IDE: TDxIDE;
      const Value: TDxThirdPartyComponents);
    ...
    procedure InstallPackage(IDE: TDxIDE; const IDEPlatform: TDxIDEPlatform;
      Component: TDxComponent; Package: TDxPackage);
    procedure Uninstall(IDE: TDxIDE); overload;
    procedure UninstallPackage(IDE: TDxIDE; const IDEPlatform: TDxIDEPlatform;
      Component: TDxComponentProfile; const PackageBaseName: String);
    procedure UpdateProgress(IDE: TDxIDE; Component: TDxComponentProfile;
  ...
  public
    ...
    property InstallFileDir: String read FInstallFileDir
      write SetInstallFileDir;
    property Components[IDE: TDxIDE]: TDxComponentList read GetComponents;
    property Options[IDE: TDxIDE]: TDxInstallOptions read GetOptions
      write SetOptions;
    property ThirdPartyComponents[IDE: TDxIDE]: TDxThirdPartyComponents
      read GetThirdPartyComponents write SetThirdPartyComponents;
    property State: TDxInstallerState read FState;
    property OnUpdateProgress: TDxUpdateProgressEvent read FOnUpdateProgress
      write FOnUpdateProgress;
    property OnUpdateProgressState: TDxUpdateProgressStateEvent
      read FOnUpdateProgressState write FOnUpdateProgressState;
    ...
    class function GetInstallLibraryDir(const InstallFileDir: String;
      IDE: TDxIDE; const IDEPlatform: TDxIDEPlatform = Win32): String;
    class function GetInstallSourcesDir(const InstallFileDir: String): String;
  end;

const
  DxInstallOptionNames: array [TDxInstallOption] of String =
    ('Add Browsing Path', 'Use Native Look and Feel as Default',
    'Compile Win64 Library', 'Install to C++Builder', 'Debug');
...
begin
  ...
  for I := 0 to FIDEs.Count - 1 do
    FIDEs[I].OutputCallback := UpdateProgressState;
  ...
  for I := 0 to FIDEs.Count - 1 do
    DetectionThirdPartyComponents(FIDEs[I]);
  FState := dxisNormal;
end;
...
begin
  Components := [];
  for I := 0 to IDE.IdePackages.Count - 1 do
  begin
    FileName := IDE.IdePackages.PackageFileNames[I];
    if (not(dxtpcIBX in Components)) and (Pos('\dclib', FileName) > 0) then
      Include(Components, dxtpcIBX)
    else if (not(dxtpcTeeChart in Components)) and (Pos('\dcltee', FileName) > 0)
    then
      Include(Components, dxtpcTeeChart)
    else if (not(dxtpcFireDAC in Components)) and
      ((Pos('\dclFireDAC', FileName) > 0) or (Pos('\AnyDAC_', FileName) > 0))
    then
      Include(Components, dxtpcFireDAC)
    else if (not(dxtpcBDE in Components)) and (Pos('\dclbde', FileName) > 0)
    then
      Include(Components, dxtpcBDE);
  end;
  ThirdPartyComponents[IDE] := Components;
end;

class function TDxInstaller.GetInstallLibraryDir(const InstallFileDir: String;
  IDE: TDxIDE; const IDEPlatform: TDxIDEPlatform): String;
begin
  Result := IncludeTrailingPathDelimiter(InstallFileDir) + 'Library';
  if IDE <> nil then
  begin
    Result := Result + '\' + TDxProfile.GetIDEVersionNumberStr(IDE);
    if IDEPlatform = Win64 then
      Result := Result + '\' + DxIDEPlatformNames[IDEPlatform];
  end;
end;

class function TDxInstaller.GetInstallSourcesDir(const InstallFileDir
  : String): String;
begin
...
begin
  Options := Value;
  if (dxioCompileWin64Library in Options) and (not IsSupportWin64(IDE)) then
    Exclude(Options, dxioCompileWin64Library);
  if (dxioInstallToCppBuilder in Options) and (not IsSupportCppBuilder(IDE))
  then
    Exclude(Options, dxioInstallToCppBuilder);
  FOptions[IDEs.IndexOf(IDE)] := Options;
end;

function TDxInstaller.GetThirdPartyComponents(IDE: TDxIDE)
  : TDxThirdPartyComponents;
begin
  Result := FThirdPartyComponents[IDEs.IndexOf(IDE)];
end;

procedure TDxInstaller.SetThirdPartyComponents(IDE: TDxIDE;
  const Value: TDxThirdPartyComponents);
begin
...
begin
  List.Clear;
  if InstallFileDir = EmptyStr then
    Exit;

  Packages := TStringList.Create;
  DPKFileList := TStringList.Create;
  try
    for Component in Profile.Components do
    begin
    ...
    BuildFileList(IncludeTrailingPathDelimiter(InstallFileDir) + '*.dpk',
      DPKFileList, faAnyFile, True, True);
    for S in DPKFileList do
    begin
      FileName := ChangeFileExt(ExtractFileName(S), '');
      for I := Length(FileName) downto 1 do
        if CharInSet(FileName[I], ['0' .. '9']) then
          Delete(FileName, I, 1)
        else
          Break;
      if SameText(FileName[Length(FileName)], 'D') then
        FileName := Copy(FileName, 1, Length(FileName) - 1)
      else if SameText(Copy(FileName, Length(FileName) - 1, 2), 'RS') then
        FileName := Copy(FileName, 1, Length(FileName) - 2);
      if Packages.IndexOf(FileName) < 0 then
        List.Add(S);
    end;
  finally
    Packages.Free;
...
begin
  ...
  try
    for I := 0 to IDEs.Count - 1 do
    begin
      if FComponents[I] = nil then
        FComponents[I] := TDxComponentList.Create;
  ...
  for I := 0 to IDEs.Count - 1 do
    Options[IDEs[I]] := [dxioAddBrowsingPath, dxioNativeLookAndFeel];
end;
...
begin
  if State = Value then
    Exit;
  if (Value = dxisStopped) and (State = dxisNormal) then
    Exit;
  FState := Value;
  case State of
    dxisNormal:
      UpdateProgressState('Finished!');
    dxisStopped:
      UpdateProgressState('Stopped.');
    dxisError:
      UpdateProgressState('Error.');
  end;
  if State = dxisError then
    SetState(dxisRunning); // On Ignore Error.
end;
...
var
  ...
  procedure AddLibrarySearchPath(const Dir: String;
    const IDEPlatform: TDxIDEPlatform);
  begin
    IDE.AddToLibrarySearchPath(Dir, IDEPlatform);
    if (dxioInstallToCppBuilder in Options[IDE]) and IsRADStudio(IDE) then
      TDxBDSIDE(IDE).AddToCppLibraryPath(Dir, IDEPlatform);
  end;
  procedure AddLibraryBrowsingPath(const Dir: String;
    const IDEPlatform: TDxIDEPlatform);
  begin
    IDE.AddToLibraryBrowsingPath(Dir, IDEPlatform);
    if (dxioInstallToCppBuilder in Options[IDE]) and IsRADStudio(IDE) then
      TDxBDSIDE(IDE).AddToCppBrowsingPath(Dir, IDEPlatform);
  end;
  procedure CopyFiles(const ASourcesFileDir: String);
  begin
    CopyFilesToDirectory(ASourcesFileDir + '*.*', InstallSourcesDir);
    CopyFilesToDirectory(ASourcesFileDir + '*.dfm;*.res',
      GetInstallLibraryDir(InstallFileDir, IDE, Win32));
    if dxioCompileWin64Library in Options[IDE] then
      CopyFilesToDirectory(ASourcesFileDir + '*.dfm;*.res',
        GetInstallLibraryDir(InstallFileDir, IDE, Win64));
  end;
...
begin
  ...
  begin
    if Comp.State <> dxcsInstall then
      Continue;
    SourcesFileDir := IncludeTrailingPathDelimiter
      (Profile.GetComponentSourcesDir(InstallFileDir,
      Comp.Profile.ComponentName));
    ...
    if (dxBuildNumber >= 20180200) and
      (Comp.Profile.ComponentName = 'ExpressLibrary') then
    begin
      for I := 0 to Profile.Components.Count - 1 do
        if DirectoryExists(Profile.GetComponentSourcesDir(InstallFileDir,
          Profile.Components[I].ComponentName)) and
          not DirectoryExists(Profile.GetComponentPackagesDir(InstallFileDir,
          Profile.Components[I].ComponentName)) then
          CopyFiles(IncludeTrailingPathDelimiter(Profile.GetComponentSourcesDir
            (InstallFileDir, Profile.Components[I].ComponentName)));
      CopyFiles(IncludeTrailingPathDelimiter(Profile.GetComponentSourcesDir
        (InstallFileDir, 'ExpressPageControl')));
    end;
  ...
  if dxioCompileWin64Library in Options[IDE] then
    AddLibrarySearchPath(GetInstallLibraryDir(InstallFileDir, IDE,
      Win64), Win64);
  if dxioAddBrowsingPath in Options[IDE] then
  begin
    AddLibraryBrowsingPath(InstallSourcesDir, Win32);
    if dxioCompileWin64Library in Options[IDE] then
      AddLibraryBrowsingPath(InstallSourcesDir, Win64);
  end
  else
  begin
    AddLibrarySearchPath(InstallSourcesDir, Win32);
    if dxioCompileWin64Library In Options[IDE] then
      AddLibrarySearchPath(InstallSourcesDir, Win64)
  end;

  SetIDEOverrideEnvironmentVariable(IDE, DxEnvironmentVariableName,
    InstallFileDir);
end;

procedure TDxInstaller.InstallPackage(IDE: TDxIDE;
  const IDEPlatform: TDxIDEPlatform; Component: TDxComponent;
  Package: TDxPackage);
var
...
begin
  ...
  case Package.Category of
    dxpcIBX:
      if not(dxtpcIBX in ThirdPartyComponents[IDE]) then
        Exit;
    dxpcTeeChart:
      if not(dxtpcTeeChart in ThirdPartyComponents[IDE]) then
        Exit;
    dxpcFireDAC:
      if not(dxtpcFireDAC in ThirdPartyComponents[IDE]) then
        Exit;
    dxpcBDE:
      if not(dxtpcBDE in ThirdPartyComponents[IDE]) or (IDEPlatform = Win64)
      then
        Exit;
  end;
  if (IDEPlatform = Win64) and (not(dxioCompileWin64Library in Options[IDE]))
  then
    Exit;
  if (IDEPlatform = Win64) and (Package.Usage = dxpuDesigntimeOnly) then
    Exit;
  if not Package.Required then
    for I := 0 to Package.DependentComponents.Count - 1 do
      if Package.DependentComponents[I].State <> dxcsInstall then
        Exit;

  UpdateProgress(IDE, Component.Profile, 'Install Package',
    DxIDEPlatformNames[IDEPlatform] + ' > ' + Package.FullFileName);
  if IDEPlatform = Win32 then
    IDE.DCC := IDE.DCC32
  else if IDEPlatform = Win64 then
    TDxBDSIDE(IDE).DCC := TDxBDSIDE(IDE).DCC64;
  ...
  vIsDebug := dxioDebug in Options[IDE];
//  if vIsDebug then
//  begin
//    InstallSourcesDir := InstallSourcesDir + '\Debug';
//    InstallLibraryDir := InstallLibraryDir + '\Debug';
//  end;
  ...
  ExtraOptions := ' -$D- -$L- -$Y- -Q ' + Format(' -U"%s" -U"%s" -R"%s" ',
    [DCPPath, InstallSourcesDir, InstallSourcesDir]) +
  ...
  if vIsDebug then
    ExtraOptions := ExtraOptions + ' -DDXDEBUGMODE ';
  if dxioNativeLookAndFeel in Options[IDE] then
    ExtraOptions := ExtraOptions + ' -DUSENATIVELOOKANDFEELASDEFAULT ';
  if dxioInstallToCppBuilder in Options[IDE] then
    ExtraOptions := ExtraOptions + Format(' -JL -NB"%s" -NH"%s" -NO"%s" ',
      [DCPPath, InstallLibraryDir, DCPPath]);

  R := IDE.CompileDelphiPackageEx(Package.FullFileName, BPLPath, DCPPath,
    ExtraOptions, vIsDebug);
  if R then
  begin
    // Fix issue that the skin names not listed since v18.2.x. dxSkinXxxxx.bpl should be placed in the library install directory.
    if Package.Name.StartsWith('dxSkin') and CharInSet(Package.Name.Chars[6],
      ['A' .. 'Z']) then
      CopyFile(IncludeTrailingPathDelimiter(BPLPath) + Package.Name +
        BPLExtName, IncludeTrailingPathDelimiter(InstallLibraryDir) +
        Package.Name + BPLExtName, True);

    if (IDEPlatform = Win32) and (Package.Usage <> dxpuRuntimeOnly) then
      R := IDE.RegisterPackage(Package.FullFileName, BPLPath,
        Package.Description);
  end;
  if not R then
    SetState(dxisError);
...
var
  ...
  begin
    ...
    if IsRADStudio(IDE) and IsSupportCppBuilder(IDE) then
    begin
...
begin
  ...
  InstallFileDir := GetIDEOverrideEnvironmentVariable(IDE,
    DxEnvironmentVariableName);
  if InstallFileDir = EmptyStr then
    Exit;
  ...
  if IsEmptyDirectory(InstallLibraryDir, InstallSourcesDir) then
  begin
...
procedure TDxInstaller.UninstallPackage(IDE: TDxIDE;
  const IDEPlatform: TDxIDEPlatform; Component: TDxComponentProfile;
  const PackageBaseName: String);
var
  BPLPath, DCPPath, PackageName, FileName: String;
begin
  CheckStoppedState;
  if (IDEPlatform = Win64) and (not IsSupportWin64(IDE)) then
    Exit;
...
procedure TDxInstaller.UpdateProgress(IDE: TDxIDE;
  Component: TDxComponentProfile; const Task, Target: String);
begin
  if Assigned(FOnUpdateProgress) then
    FOnUpdateProgress(IDE, Component, Task, Target);
end;

procedure TDxInstaller.UpdateProgressState(const StateText: String);
begin
  if Assigned(FOnUpdateProgressState) then
    FOnUpdateProgressState(StateText)
end;
