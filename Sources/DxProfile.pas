...
type
  ...
  public
    ...
    class function GetComponentDir(const InstallFileDir, ComponentName
      : String): String;
    class function GetComponentSourcesDir(const InstallFileDir,
      ComponentName: String): String;
    class function GetComponentPackagesDir(const InstallFileDir,
      ComponentName: String): String;
    class function GetPackageName(const PackageBaseName: String;
      IDE: TDxIDE): String;
    class function GetPackageFullFileName(const InstallFileDir, ComponentName,
      PackageBaseName: String; IDE: TDxIDE): String;
    class function GetDxBuildNumber(const InstallFileDir: String): Cardinal;
    class function GetDxBuildNumberAsVersion(const BuildNumber
      : Cardinal): String;
  end;
...
begin
  ...
  try
    ...
    begin
      ...
      StrToList(Ini.ReadString(Name, RequiredPackagesIdent, ''),
        Comp.RequiredPackages);
      StrToList(Ini.ReadString(Name, OptionalPackagesIdent, ''),
        Comp.OptionalPackages);
      StrToList(Ini.ReadString(Name, OutdatedPackagesIdent, ''),
        Comp.OutdatedPackages);
...
begin
  Result := Format('%s%s.%s', [ExtractFilePath(Application.ExeName),
    ProfileResourceName, ProfileResourceType]);
end;

class function TDxProfile.GetComponentDir(const InstallFileDir,
  ComponentName: String): String;
begin
  Result := IncludeTrailingPathDelimiter(InstallFileDir) + ComponentName;
end;

class function TDxProfile.GetComponentSourcesDir(const InstallFileDir,
  ComponentName: String): String;
begin
  Result := GetComponentDir(InstallFileDir, ComponentName) + '\Sources';
end;

class function TDxProfile.GetComponentPackagesDir(const InstallFileDir,
  ComponentName: String): String;
begin
  Result := GetComponentDir(InstallFileDir, ComponentName) + '\Packages';
end;

class function TDxProfile.GetPackageName(const PackageBaseName: String;
  IDE: TDxIDE): String;
begin
  Result := PackageBaseName + GetIDEVersionNumberStr(IDE);
end;

class function TDxProfile.GetPackageFullFileName(const InstallFileDir,
  ComponentName, PackageBaseName: String; IDE: TDxIDE): String;
begin
...
class function TDxProfile.GetDxBuildNumber(const InstallFileDir: String)
  : Cardinal;
const
...
begin
  Result := 0;
  S := IncludeTrailingPathDelimiter(InstallFileDir) +
    'ExpressCore Library\Sources\dxCore.pas';
  ...
  begin
    ReadLn(SourceFile, S);
    S := S.Trim;
    if S.StartsWith(VersionIdent) or S.StartsWith(BuildNumberIdent) then
    begin
...
class function TDxProfile.GetDxBuildNumberAsVersion(const BuildNumber
  : Cardinal): String;
var
  I: Integer;
begin
  ...
  begin
    ...
    Result := Format('%d.%d.%d', [BuildNumber div 10000 mod 100, I div 100,
      I mod 100]);
  end
  else
    Result := 'n/a';
...
begin
  ...
  if IsRADStudio(IDE) then
    Result := StringReplace(Result, 'd', 'rs', [rfIgnoreCase]);
